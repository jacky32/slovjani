...
class AdminAttachmentsController extends AdminController
{
  ...
  public function __construct()
  {
    ...
    preg_match('/\/admin\/(posts|users|events)\/(\d+)\/attachments(?:\/(\d+))?/', $_SERVER['REQUEST_URI'], $matches);
  ...
  public function show($request)
  {
    ...
    if ($attachment) {
      ...
      $this->addFlash('error', "Příloha neexistuje.");
  ...
  public function create($request)
  {
    try {
      ...
      $attachment->save();
      $this->addFlash('success', "Příloha byla úspěšně vytvořena.");
      header("Location: /admin/{$this->resource_type}/{$this->resource_id}");
    } catch (Exception $e) {
      $errors = [];
  ...
  public function destroy($request)
  {
    try {
      ...
      if ($attachment && $attachment->creator_id == $this->auth->getUserId()) {
        $attachment->destroy();
        $this->addFlash('success', "Příloha byla úspěšně smazána.");
      } else {
        if (!$attachment) {
          $this->addFlash('error', "Příloha neexistuje.");
        } else if ($attachment->creator_id != $this->auth->getUserId()) {
          $this->addFlash('error', "Nemáte oprávnění smazat tuto přílohu.");
        }
